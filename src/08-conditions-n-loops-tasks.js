...
function findFirstSingleChar(str) {
  ...
  arrStr.forEach((element) => {
    if ((!nonRep) && (arrStr.filter((element1) => element === element1).length === 1)) {
      nonRep = element;
    }
  });
  return nonRep;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let representationInterval = '';
  const firstCount = Math.min(a, b);
  const finishCount = Math.max(a, b);
  representationInterval = isStartIncluded ? `[${firstCount}, ` : `(${firstCount}, `;
  representationInterval = isEndIncluded ? `${representationInterval}${finishCount}]` : `${representationInterval}${finishCount})`;
  return representationInterval;
}
...
function getCommonDirectoryPath(pathes) {
  const arrCommomDirectory = [];
  const pathesArr = pathes.map((value) => value.split('/'));
  let minElem = Infinity;
  pathesArr.forEach((element) => {
    minElem = Math.min(minElem, element.length);
  });
  for (let index = 0; index < minElem; index += 1) {
    let preElem = '';
    let counSameValue = 0;
    pathesArr.forEach((element, index1) => {
      if (index1 === 0 || preElem === element[index]) {
        preElem = element[index];
        counSameValue += 1;
      }
    });
    if (counSameValue === pathesArr.length) {
      arrCommomDirectory.push(preElem);
    }
  }
  arrCommomDirectory.push('');
  return arrCommomDirectory.join('/');
}
